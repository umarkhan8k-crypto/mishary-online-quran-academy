...
async function handleApi(request, env, url, ctx) {
  ...
  if (path === '/api/book-trial' && method === 'POST') {
    try {
      const data = await request.json();

      // میں تفصیل اور ٹیوٹر کے میسج کو سیو کرنے کی کوشش کرتی ہیں (D1 یا KV میں) — یہاں آپ ڈیٹا
      // مثال کے طور پر:
      // await env.DB.prepare("INSERT INTO messages (tutor_id, student_name, course, message, time) VALUES (?, ?, ?, ?, ?)")
